...
var format = {
  ...
  custom_params: ({provider, provider: {custom_parameters = []}, provider: {custom_params = {}}}) => {

    var params = custom_parameters
      .filter((key) => !reserved.includes(key) && key !== provider.name)
      .reduce((params, key) => (params[key] = provider[key], params), custom_params)
  ...
  overrides: ({provider, provider: {custom_parameters = []}}) => {
    var keys = Object.keys(provider)
      .filter((key) =>
        !reserved.includes(key) &&
        !custom_parameters.includes(key) &&
        typeof provider[key] === 'object'
      )

    var overrides = keys
      .reduce((all, key) => (all[key] = provider[key], all), {})
...
var transform = (provider) => {
  ...
  ;['oauth', 'key', 'secret', 'scope', 'state', 'nonce', 'redirect_uri']
    .reduce((all, key) => (all[key] = format[key](provider), all), provider)

  ;['custom_params', 'overrides']
    .reduce((all, key) => (all[key] = format[key]({provider}), all), provider)
