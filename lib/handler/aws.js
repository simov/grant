...
module.exports = function (args = {}) {
  ...
  var store = Session({...args.session, handler: 'aws'})

  async function app (req, state) {
    var session = store(req)
    var match = regex.exec(req.requestContext.path.replace(`/${req.requestContext.stage}`, ''))
    if (!match) {
      return {session}
    }
...
var redirect = (location, session) => JSON.parse(JSON.stringify({
  statusCode: 302,
  headers: {location},
