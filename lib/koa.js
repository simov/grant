...
function Grant (_config) {
  ...
  app.get('/connect/:provider/callback', function *(next) {
    var grant = this.session.grant
    var provider = p(app.config, grant)
    var flow = flows[provider.oauth]

    if (provider.oauth == 1) {
      // this.response.type = 'application/x-www-form-urlencoded; charset=utf-8'
      var url
      try {
  ...
  // app.get(/\/connect\/(\w+)(?!\/callback)(\/\w+)?/, function *(next) {
    // if (this.params.override == 'callback') return

    this.session.grant = {
      provider:this.params.provider,
      override:this.params.override
      // provider:this.params[0],
      // override:this.params[1]
    }
