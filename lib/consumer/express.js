...
module.exports = function (_config, extraConfig) {
  var app = express()
  app.config = config(_config)

  if(extraConfig) {
    app.config.isCookieSession = extraConfig.isCookieSession
  }
  ...
  var redirect = (req, res, url) => {
    ...
    if(app.config.isCookieSession || typeof req.session.save !== 'function') {
      res.redirect(url)
      return
    }
